package ru.dkarelin.myosmdroidmapsforge_1.osm;
...
import ru.dkarelin.myosmdroidmapsforge_1.R;
...
//        mapView.getOverlays().add(scaleBarOverlay);
    ...
    // TODO: Octane DEBUG method (add marker)
    private void drawMarker() {
        GeoPoint startPoint = new GeoPoint(55.571527,38.230545);
        Marker startMarker = new Marker(this);
        startMarker.setPosition(startPoint);
        startMarker.setIcon(getResources().getDrawable(R.mipmap.ic_launcher));
        startMarker.setTitle("Start point");
        startMarker.setAnchor(Marker.ANCHOR_CENTER, Marker.ANCHOR_BOTTOM);
        this.getOverlays().add(startMarker);

        // map.invalidate();
    }

    // TODO: Octane DEBUG method (add route)
    private void drawRoute() {
        ...
        GeoPoint endPoint = new GeoPoint(55.567344,38.222262);
        waypoints.add(endPoint);
        Road road = roadManager.getRoad(waypoints);
        if (road.mStatus != Road.STATUS_OK)
            Toast.makeText(getContext(), "Error when loading the road - status="+road.mStatus, Toast.LENGTH_SHORT).show();
    ...
    public void setCurrentAsCenter()
    {
        ...
        else
        {
            ...
            else
            {
                ...
                this.getController().setCenter(new GeoPoint(55.568925,38.225181)); // Remenskoye
                this.getController().setZoom(15);
            }
            try
            {
